...
def send_result(results):
    ...
    with open(f'{outfile}{FILE_NAME}.csv', 'w', newline='') as csvfile:
        ...
        if len(w_['date']) > 0:
            ...
            cprint(f'\nNo Transactions since {LAST_DATE_TX} :', color)
        ...
        if len(w_['tx_amount']) > 0:
            ...
            cprint(f'\nNumber of Transactions < {MIN_TX_AMOUNT} :', color)
        ...
        if len(w_['amount_chains']) > 0:
            ...
            cprint(f'\nConnected networks < {MIN_AMOUNT_CHAINS} :', color)
        ...
        if len(w_['days_amount']) > 0:
            ...
            cprint(f'\nNumber of days between first and last transaction < {DAYS_AMOUNT} :', color)
        ...
        for items in w_['tx_amount_chains'].items():
            ...
            if len(wallets) > 0:
                ...
                cprint(f'\nNumber of Transactions in {chain} < {MIN_TX_AMOUNT_CHAINS[chain]} :', color)
        ...
        for items in w_['tx_amount_protocols'].items():
            ...
            if len(wallets) > 0:
                ...
                cprint(f'\nNumber of Transactions in {name} < {MIN_TX_AMOUNT_PROTOCOLS[name]} :', color)
        ...
        cprint(f'\nResult into : {outfile}{FILE_NAME}.csv\n', 'blue')
